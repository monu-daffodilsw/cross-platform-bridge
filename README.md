...
mkdir -p .claude/skills/cross-platform-bridge
curl -o .claude/skills/cross-platform-bridge/SKILL.md \
  https://raw.githubusercontent.com/monu-daffodilsw/cross-platform-bridge/main/skills/cross-platform-bridge/SKILL.md
```
...
1. Download `skills/cross-platform-bridge/SKILL.md` from this repo
2. Place it at one of these paths:
   - **Project-only:** `<your-project>/.claude/skills/cross-platform-bridge/SKILL.md`
   - **Global:** `~/.claude/skills/cross-platform-bridge/SKILL.md`
...
Replace all Next.js file-based routing with `@ultra-ui-library/router`. Keeps the web app running exactly as before — just changes how routing works internally.

**Paste into Claude Code:**

> Phase 1 — Router. Read the project structure first. Replace all Next.js file-based routing with `@ultra-ui-library/router`. Keep only `app/layout.tsx` and `app/globals.css` — delete all other files inside `app/`. Create a single `app/[[...slug]]/page.tsx` catch-all. Create `router/routes.ts` with all existing routes. Replace every routing import from `next/navigation` and `next/link` across all components and hooks. Do not change any UI, styles, or dependencies. Stop when done and wait for confirmation before Phase 2.
...
Replace all HTML elements with `@ultra-ui-library` components and fix all class names to be NativeWind-compatible. After this phase the codebase is cross-platform ready.

**Paste into Claude Code:**

> Phase 2 — Primitives. Phase 1 is complete — routing is already migrated to `@ultra-ui-library/router`. Read the project structure first. Replace every HTML element in shared components with the `@ultra-ui-library` equivalent. Audit flex direction on every `div`→`View` migration. Wrap every bare JSX string in `<Text>`. Remove all `StyleSheet.create` and static `style={{ }}` objects — use `className` instead. Prefix web-only classes with `web:`. Replace all direct `localStorage` and `AsyncStorage` calls with `Storage` from `@ultra-ui-library`. Do not touch `app/` or any `.native.tsx` file. Stop when done and wait for confirmation before Phase 3.
...
|---|---|---|
| 1 | No HTML in shared code | `div`, `span`, `button`, etc. → use `@ultra-ui-library` |
| 2 | No react-native imports in shared code | `import from 'react-native'` in non-.native. files |
| 3 | No next/navigation or expo-router in shared code | use `@ultra-ui-library/router` |
...
- `dependencies/ultra-ui-library/**` — library
- `node_modules/**`
